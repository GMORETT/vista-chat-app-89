...
export const MainSidebar: React.FC = () => {
  ...
  const getNavClassName = (isActiveRoute: boolean) => `flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition-all ${isActiveRoute ? "bg-primary text-primary-foreground shadow-sm font-medium" : "text-muted-foreground hover:bg-accent hover:text-accent-foreground"}`;
  return <Sidebar className={collapsed ? "w-16" : "w-64"} collapsible="icon">
      <SidebarHeader className="border-b border-border/40 p-0">
        <div className={`flex items-center gap-3 p-4 ${collapsed ? 'justify-center' : ''}`}>
          <img src="/assets/logo-solabs-white.png" alt="Solabs" className={`${collapsed ? 'h-8 w-8' : 'h-10 w-auto'} object-contain`} />
          {!collapsed}
        </div>
      </SidebarHeader>
