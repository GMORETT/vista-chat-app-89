...
import { Building2, Globe, Calendar, User, TrendingUp } from 'lucide-react';
...
export const CompanyDetailsModal: React.FC<CompanyDetailsModalProps> = ({
  ...
  const { deals, dealStages, getLeadById } = useCrmDataStore();
  const navigate = useNavigate();

  if (!company) return null;

  const companyDeals = deals.filter(d => d.companyId === company.id);
  const lead = getLeadById(company.leadId);
  ...
  const totalDealsRevenue = companyDeals.reduce((sum, deal) => sum + deal.annualRevenue, 0);
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  Valor total: {formatCurrency(totalDealsRevenue)}
                </p>
              </CardHeader>
              <CardContent>
